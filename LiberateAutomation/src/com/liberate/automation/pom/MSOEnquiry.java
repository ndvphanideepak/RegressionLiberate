...
public class MSOEnquiry {
	...
	/***
	 * Can Be used to get the count of row of Department Circulation. Can Append
	 * '[<i>count</i>]' for a particular row. After '<i>count</i>' append <br>
	 * '<i>/descendant::td[1]</i>' for Department <br>
	 * '<i>/descendant::td[2]</i>' for From Date <br>
	 * '<i>/descendant::td[3]</i>' for To Date <br>
	 * '<i>/descendant::td[4]</i>' for Status <br>
	 * '<i>/descendant::td[5]</i>' for Created By <br>
	 * '<i>/descendant::td[6]</i>' for Actioned By <br>
	 * '<i>/descendant::td[7]</i>' for Signed Off By <br>
	 * '<i>/descendant::td[8]</i>' for Status Reason
	 */
	 ...
	 * 
	 * @return True : If able to Navigate. <br>
	 * 		False : If failed to Navigate.
	 */
	 ...
	 * @return True : If able to search. <br>
	 * 		False : If search for service order is failed.
	 */
	 ...
	 * 
	 * @param serviceOrderNumber
	 *            The service Order to be searched for.
	 * @return
	 */
	public boolean verifyServiceOrderDetails(String serviceOrderNumber) {
		action.getScreenShot("ManageServiceOrder_soEnquiry");
		this.ServiceOrderNumber = serviceOrderNumber;
		action.getScreenShot("ManageServiceOrder_soEnquiry");
		boolean passed = false;
		action.getScreenShot("ManageServiceOrder_soEnquiry");
	...
	/***
	 * Method used to get all the departments the Service Order Currently belongs
	 * to.
	 * 
	...
	public List<String> getCurrentDepartments() {
		...
		for (int i = 0; i < count; i++) {
			// Checking if action date is blank. If Date is blank, then Service order
			// belongs to that department.
			String actionDate = action
					.getTextFromPage(
							By.xpath((action.getXpath(Circulation_Row) + "[" + (i + 1) + "]" + "/descendant::td[3]")))
					.trim();
			if (actionDate.equals("")) {
				// Getting the department name (cell value) where the action date is blank.
				CurrentDepartments.add(action
						.getTextFromPage(By
								.xpath((action.getXpath(Circulation_Row) + "[" + (i + 1) + "]" + "/descendant::td[1]")))
						.trim());
			}
		}
