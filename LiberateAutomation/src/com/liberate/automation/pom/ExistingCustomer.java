...
//Product Fieldss
	...
	By HomeRateRoaming_textbox = By.id("packageProductSelection:productFieldDetails:3:productFieldValue");
	// ISP Field
	...
	public boolean selectDepartmentSite(String Department, String Site) {
		...
		if (action.countOf(applicationsource_select) > 0) {
			...
			passed = action.selectBy(applicationsource_select, 1);
		}
		
		action.selectBy(applicationsource_select, 1);
		return passed;
	}

	public boolean applicationsource()
	{
		By applicationsource_xpath = By.xpath("//*[text() = 'Application Source:']//following::select[1]");
		boolean passed = false;
		
		
		action.selectBy(applicationsource_xpath, 1);
	...
	public boolean selectServicePackage_PDL(String ServiceType, String ServicePackage) {
		boolean passed = false;
		...
		By package_dropdown = By.xpath("//*[text() = 'Service Package:']//following::Select");
		action.sendDataTo(package_dropdown, "ADSL Test- Testing(PDL)");
		action.waitFor(1);
		passed = action.clickOn(Proceed_Button);
...
//		By bundels_Select = By.xpath("//*[text()='Bundles:']//following::select[1]");
	...
	public boolean processProductFieldsScreen() {
		...
		action.sendDataTo(HomeRateRoaming_textbox, "dee");
		action.scrollUp();
		action.getTextFromPage(Panel_Header);
		action.clickOn(Proceed_Button);
	...
	public boolean processISPFieldsScreen() {
		...
		action.sendDataTo(ISPUserName_Input, "Deepak");
		action.waitFor(2);
		action.clickOn(GeneratePassword_Button);
	...
	public boolean provideServiceDetailsScreen(String Exchange, String NumberArea) {
		...
		By Allocate = By.xpath("//*[@value = 'Allocate']");
		...
		action.selectByPartialText(Exchange_Select, Exchange);
		action.waitFor(2);
		passed = action.waitFor(NumberAreaDisabled_Select, 4, false);
		action.waitFor(2);
		passed = action.selectByPartialText(NumberArea_Select, NumberArea);
...
//	
	...
	{
		By addresschangedropdown = By.xpath("//*[text() = 'Address Type:']//following::select[1]");
		By billarea = By.xpath("//*[text() = 'Bill Stats Area:']//following::select[1]");
		By searchclick = By.xpath("//*[text() = 'Bill Stats Area:']//following::input[2]");
		boolean passed = false;
		action.selectBy(addresschangedropdown, "A-Standard Address123");
		action.waitFor(3);
		
		action.clickOn(searchclick);
		action.waitFor(2);
		action.selectBy(billarea, 2);
		action.waitFor(2);
		return passed;
	}
